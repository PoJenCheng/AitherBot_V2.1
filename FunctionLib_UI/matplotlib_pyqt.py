...
class MainWidget(QMainWindow, FunctionLib_UI.ui_matplotlib_pyqt.Ui_MainWindow, MotorSubFunction):
    ...
    def ImportDicom_L(self):
        ...
        self.dcmLow.update({"imageTag":self.dcmFn.ReadDicom(seriesNumberLabel, dicDICOM)})
        ...
        self.dcmLow.update({"imageHu":numpy.array(self.dcmFn.Transfer2Hu(self.dcmLow.get("image"), rescaleSlope, rescaleIntercept))})
        self.dcmLow.update({"pixel2Mm":self.dcmFn.GetPixel2Mm(self.dcmLow.get("imageTag")[0])})
        ...
        self.SliceSelect_Axial_L.setMaximum(self.dcmLow.get("imageHuMm").shape[0]-1)
        self.SliceSelect_Axial_L.setValue((self.dcmLow.get("imageHuMm").shape[0])/2)
        self.SliceSelect_Sagittal_L.setMinimum(0)
        self.SliceSelect_Sagittal_L.setMaximum(self.dcmLow.get("imageHuMm").shape[1]-1)
        self.SliceSelect_Sagittal_L.setValue((self.dcmLow.get("imageHuMm").shape[1])/2)
        self.SliceSelect_Coronal_L.setMinimum(0)
        self.SliceSelect_Coronal_L.setMaximum(self.dcmLow.get("imageHuMm").shape[2]-1)
        self.SliceSelect_Coronal_L.setValue((self.dcmLow.get("imageHuMm").shape[2])/2)
        ...
        self.dcmLow.update({"ww":int(self.dcmLow.get("imageTag")[0].WindowWidth[0])})
        self.dcmLow.update({"wl":int(self.dcmLow.get("imageTag")[0].WindowCenter[0])})
    ...
    def ImportDicom_H(self):
        ...
        seriesNumberLabel, dicDICOM = self.dcmFn.SeriesSort(metadata, metadataSeriesNum)
        self.dcmHigh.update({"imageTag":self.dcmFn.ReadDicom(seriesNumberLabel, dicDICOM)})
        ...
        self.dcmHigh.update({"imageHu":numpy.asarray(self.dcmFn.Transfer2Hu(self.dcmHigh.get("image"), rescaleSlope, rescaleIntercept))})
        self.dcmHigh.update({"pixel2Mm":self.dcmFn.GetPixel2Mm(self.dcmHigh.get("imageTag")[0])})
        ...
        self.SliceSelect_Axial_H.setMaximum(self.dcmHigh.get("imageHuMm").shape[0]-1)
        self.SliceSelect_Axial_H.setValue((self.dcmHigh.get("imageHuMm").shape[0])/2)
        self.SliceSelect_Sagittal_H.setMinimum(0)
        self.SliceSelect_Sagittal_H.setMaximum(self.dcmHigh.get("imageHuMm").shape[1]-1)
        self.SliceSelect_Sagittal_H.setValue((self.dcmHigh.get("imageHuMm").shape[1])/2)
        self.SliceSelect_Coronal_H.setMinimum(0)
        self.SliceSelect_Coronal_H.setMaximum(self.dcmHigh.get("imageHuMm").shape[2]-1)
        self.SliceSelect_Coronal_H.setValue((self.dcmHigh.get("imageHuMm").shape[2])/2)
        ...
        self.dcmHigh.update({"wl":int(self.dcmHigh.get("imageTag")[0].WindowCenter[0])})
    ...
    def SetRegistration_L(self):
        """™ÂæÁ‰∏≠Â + âÁÖßÜÂ (origin -> NO.1 -> NO.2) ∏Êball
        """
        "™ÂæÁ‰∏≠Â"
        if self.dcmLow.get("regBall") != []:
            reply = QMessageBox.information(self, "êÁ§∫", "Â∑≤ÁâË®ªä‰ÔºåËçË®≠", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        ...
        "âÁÖßÜÂ (origin -> NO.1 -> NO.2) ãÈ/∏Êball"
        try:
    ...
    def ShowRegistrationDifference_L(self):
        """äÊâæ∞Á‰∏≠ÂÈªûËãÈÑmappingËµ∑‰
           ÁÆóÁõ∏Â∞çË™§Â∑ÆËerror difference)
        """
        "äÊâæ∞Á‰∏≠ÂÈªûËãÈ/∏ÊÑÁmapping / PairçÂËµ∑‰"
        ...
        if selectedBall != []:
            ...
            if flagePair == True:
                ...
                self.logUI.info(logStr)
                self.label_error_L.setText('Error / Difference: {:.2f} mm'.format(error[2]))
                "ÁÆóËõÁü©
                regMatrix = self.regFn.TransformationMatrix(self.dcmLow.get("regBall"))
    ...
    def SetPoint_L(self):
        if self.dcmLow.get("flageSelectedPoint") == False:
            ...
            reply = QMessageBox.information(self, "êÁ§∫", "Â∑≤ÁâÈÅ∏Èªû‰ÔºåËçË®≠", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
            if reply == QMessageBox.Yes:
                self.dcmLow.update({"selectedPoint":[]})
                self.dcmLow.update({"flageSelectedPoint":False})
                self.Button_Planning.setEnabled(False)
    ...
    def ShowRegistrationDifference_H(self):
        ...
        if selectedBall != []:
            ...
            if flagePair == True:
                ...
                logStr = 'registration error (min, max, mean): ' + str(error)
                self.logUI.info(logStr)
                self.label_error_H.setText('Error / Difference: {:.2f} mm'.format(error[2]))
                "ÁÆóËõÁü©
                regMatrix = self.regFn.TransformationMatrix(self.dcmHigh.get("regBall"))
                self.logUI.info('get registration matrix: ')
                for tmp in regMatrix:
                    self.logUI.info(tmp)
    ...
    def SetPoint_H(self):
        if self.dcmHigh.get("flageSelectedPoint") == False:
            if self.comboBox_H.currentText() == "Axial":
                self.ui_SPS = SetPointSystem(self.dcmHigh, self.comboBox_H.currentText(), self.SliceSelect_Axial_H.value())
                self.ui_SPS.show()
            elif self.comboBox_H.currentText() == "Coronal":
                self.ui_SPS = SetPointSystem(self.dcmHigh, self.comboBox_H.currentText(), self.SliceSelect_Coronal_H.value())
                self.ui_SPS.show()
            elif self.comboBox_H.currentText() == "Sagittal":
                self.ui_SPS = SetPointSystem(self.dcmHigh, self.comboBox_H.currentText(), self.SliceSelect_Sagittal_H.value())
                self.ui_SPS.show()
            else:
...
class CoordinateSystem(QWidget, FunctionLib_UI.ui_coordinate_system.Ui_Form):
    ...
    def  DisplayImage(self):
        ...
        if showAxis == 0:
            ...
            "pixel2Mm‰∏ÄÂ§ß‰Â∞èÁresizeÊ≤íÊÉÊÖÆ≤Âéª"
    ...
    def UpdateImage(self):
        ...
        "GetClickedPosition‰∏çÁî®), ÉÁî¢ü‰πerror: "
...
class SetPointSystem(QWidget, FunctionLib_UI.ui_set_point_system.Ui_Form):
    ...
    def UpdateImage(self):
        ...
        self.qimg = QImage(self.gray3Channel, self.imgWidth, self.imgHeight, bytesPerline, QImage.Format_RGB888).rgbSwapped()
        self.label_img.setPixmap(QPixmap.fromImage(self.qimg))
        "GetClickedPosition‰∏çÁî®ÂÆ), ÉÁî¢üerror"
        "TypeError: GetClickedPosition() missing 1 required positional argument: 'event'"
        self.label_img.mousePressEvent = self.GetClickedPosition
        return
